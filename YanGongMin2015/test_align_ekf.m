...
phi0 = [10; 10; 30*60]*glv.min; % large yaw misalignment error 大方位失准角
...
for k=1:nn:len-nn+1
    ...
    qnb = qupdt(qnb, phim-Cnb'*eth.wnie*nts); % attitide updating
    % Kalman filter
    kf = kfupdate(kf, vn(1:2));
    phi = qq2afa(qnb, qnb0); %计算的姿态角与真实姿态角的差值,即失准角
    reskf(ki,:) = [kf.xk(1:3)-phi; kf.xk(4:5); t]';%第一列表示估计的失准角与实际失准角的差值
    % EKF filter
