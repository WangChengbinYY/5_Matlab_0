...
function kf = afnkfinit(nts, pos, phi0, imuerr)  %½µ½×Ä£ÐÍ
    eth = earth(pos);
    kf = []; kf.s = 1; kf.nts = nts;
    kf.Qk = diag([imuerr.web; 0;0])^2*nts;  % web - angular random walk (deg/sqrt(h))
	kf.Rk = diag(imuerr.wdb(1:2)/sqrt(nts))^2; % wdb - velocity random walk (ug/sqrt(Hz))
