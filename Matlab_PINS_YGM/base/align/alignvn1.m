function [att0, attk,vnk, xkpk] = alignvn(imu, qnb, pos, phi0, imuerr, wvn, ts)
...
%     kf.fading = 1;
    ...
    [attk, vnk,xkpk] = prealloc(fix(len/nn), 3, 3, 2*kf.n);
    ki = 1;
    for k=1:nn:len-nn+1
...
%         vn = vn + dvn + eth.gn*nts;
           vtemp = zeros(1,3);
            for l=1:1:nn-1
                vtemp = vtemp + wvm(l,4:6)*nts;
           end;
          vn = vn + eth.gn*nts + Cnb*vtemp';
%          qnb = qupdt(qnb, phim-Cnb'*eth.wnin*nts);
        Cnbts = Cnb*nts;
        
%         kf.Phikk_1(4:6,1:3) = askew(dvn);
        kf.Phikk_1(1:3,1:3) = askew(-eth.wnie)*2*nts; 
        kf.Phikk_1(1:3,7:9) = -Cnbts; 
        kf.Phikk_1(4:6,10:12) = Cnbts;
        
        kf = kfupdate(kf, vn);
        
%         qnb = qdelphi(qnb, 0.1*kf.xk(1:3));         
%          kf.xk(1:3) = 0.9*kf.xk(1:3);        c
%         vn = vn-0.1*kf.xk(4:6);         
%          kf.xk(4:6) = 0.9*kf.xk(4:6);
        qnb = qdelphi(qnb, kf.xk(1:3));         
         kf.xk(1:3) = kf.xk(1:3);        
        vn = vn-kf.xk(4:6);         
         kf.xk(4:6) = kf.xk(4:6);        
